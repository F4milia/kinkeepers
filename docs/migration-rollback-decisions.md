...
| `20260903130000_partner_zoom_credentials` | `partner_zoom_credentials` table (service_role only) + `sessions.video_provider` (not null, default `'kinkeepers'`) | Forward-fix only | "Real data now depends on this" - every session created since this migration writes `video_provider`, and the very next migration's `finalize_cohort_sessions` requires the column to exist |
| `20260903130100_finalize_cohort_sessions_video_provider` | `drop`+`create` on `finalize_cohort_sessions` (adds `p_video_provider`, 10 params) | Forward-fix only | Function-is-the-safety-check reasoning (see `20260829120200` above) - app code calls the 10-parameter signature exclusively now; reverting to the 9-parameter version breaks every real cohort-creation call, not just future ones |
| `20260905140000_default_contact_channel_both` | `default`/backfill/`not null` on `applicants.preferred_contact_channel` | Forward-fix only | The backfill `UPDATE` already overwrote real NULL rows with `'both'` - there's no way to tell, after the fact, which rows were genuinely NULL before this ran, so reverting the constraints wouldn't restore the original (now-unknowable) state, only remove the safety net that prevents it recurring |
| `20260905150000_reminder_due_queries` | `sessions_due_for_time_reminder()`, `applicants_due_for_missed_session_followup()` (3-column return) | Forward-fix only | `sessions_due_for_time_reminder` is a pure read-only query with no side effects and would itself qualify for a down-path (same reasoning as `20260901180200`), but `applicants_due_for_missed_session_followup`'s 3-column signature here was superseded four hours later by `20260905160000` - reverting this migration without also reverting that one breaks the live Inngest reminder cron, which depends on the newer 5-column shape |
| `20260905160000_missed_session_followup_returns_instant` | `drop`+`create` on `applicants_due_for_missed_session_followup` (adds `scheduled_at`/`cohort_time_zone`, 5-column return) | Forward-fix only | This is the version the live 15-minute Inngest cron (`handleSessionReminders`) currently calls - reverting removes `scheduled_at`/`cohort_time_zone` from the return shape and breaks `notifyMissedSession()`'s day-name rendering, a functional regression, not a safe no-op |
| `20260908100000_notification_log_session_id` | nullable `notification_log.session_id` + index | Forward-fix only | Same conservative default as `20260903120000` above - a nullable additive column with no backfill drops cleanly in a fresh local reset (verified: `alter table notification_log drop column session_id` executes with zero errors in a rolled-back transaction), but this table accumulates real rows continuously in normal operation (every failed send), so a hosted drop risks losing session-linkage already surfaced on real failure records in `/admin/notifications`; not verified empty on hosted, so held to the conservative default rather than assumed safe like the row directly below |
| `20260908110000_consent_document_change_summary` | nullable `consent_documents.change_summary` | **Down-path tested**: `alter table consent_documents drop column change_summary;` executes cleanly in a rolled-back local transaction, and confirmed by grep that no code path anywhere writes to this column yet (`lib/consent/data.ts` only reads it) - unlike the row directly above, this column is empty by design (no consent document has ever been version-bumped, per the migration's own comment), not merely empty-today-in-a-fresh-reset, so it's the same safe shape as the two additive-RLS-policy rows earlier in this table |
| `20260908120000_claim_applicant_pre_enrollment` | `create or replace` on `claim_applicant_for_current_user()` (widened `where` clause, same signature) | Forward-fix only | This IS a rollback of sorts already reversed - reverting to the narrower `where` clause reintroduces the exact confirmed bug this migration fixed (a real `pending_review` applicant 404s on their first sign-in), which is a known regression, same category as `20260902120000` |
