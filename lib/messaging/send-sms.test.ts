...
describe("sendSms", () => {
  ...
  it("named edge case: no Twilio credentials configured (true in every environment as of this writing - not even the account SID/token are set, and A2P 10DLC registration is a separate pending compliance step) logs and no-ops rather than throwing", async () => {
  ...
  it("allows any recipient in production, regardless of the allowlist", async () => {
    ...
    await expect(sendSms({ to: "+15559876543", body: "x", logContext: {} })).resolves.toBeUndefined();
  });
});
